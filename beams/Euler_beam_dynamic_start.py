...
def p(x):
    ...
    x_t = x[:,1:2]
    x_s = x[:,0:1]
    return (torch.matmul(torch.sin(torch.pi*x_s),torch.transpose(torch.cos(torch.pi*x_t),1,0)))
...
ic1 = dde.IC(geomtime, lambda x: torch.sin(torch.pi*torch.from_numpy(x[:,0:1])), lambda _, on_initial: on_initial)
ic2 = dde.OperatorBC(
        geomtime,
