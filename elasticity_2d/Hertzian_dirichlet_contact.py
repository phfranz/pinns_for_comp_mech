...
def fun_gap(x, y, X):
    ...
    gap = x[:,1:2] + y[:,1:2] + distance

    return (1-tf.math.sign(gap))*gap

def fun_pressure(x,y,X):
...
bc_gap = dde.OperatorBC(geom, fun_gap, boundary_circle)
bc_pressure = dde.OperatorBC(geom, fun_pressure, boundary_circle)
bc_multip = dde.OperatorBC(geom, multip, boundary_circle)
